...
function LocalTransfer() {
  ...
  const handleAccNumChange = function (e: any) {
      ...
      (async () => {
        ...
        try {
          const response = await axios.post(
            "http://localhost:8085/api/v1/transfer/resolve-local-account",
          ...
          console.log(error);
          await swal("Please confirm the account number!!", "error");
        }
      })();
    }
  };

  const handleAmountChange = function (e: any) {
  ...
  const handlePinChange = function (e: any) {
    const value = e.target.value;
    console.log(value);
    setUser((prevState) => {
      return {
  ...
  const handleNarrationChange = function (e: any) {
    const value = e.target.value;
    console.log(value);
    setUser((prevState) => {
      return {
    ...
    LocalTransferService.saveTransaction(user, header)
      .then((response) => {
        if (response.result !== null) {
          swal("Transfer successful!!!", "success").then(function () {
    ...
    <>
      ...
      <div className="local--transfer-body">
        <div className="local--transfer-previous-page">
          ...
          <Link to="/dashboard" className="local--transfer-goBack">
            Go back
          </Link>
        </div>
        ...
        <div className="local--transfer-links">
          ...
          <p>
            {" "}
            <Link to="/bank-transfer" className="other">
              Other Bank Transfer
            </Link>
          </p>
        </div>
        ...
        <div className="local--transfer-formWrapper">
          <div className="local--transfer-formContainer">
            <form>
              <label className="local--transfer-label">Account Number</label>{" "}
              <br />
              <input
                type="text"
              ...
              />
              <label className="local--transfer-label" hidden={isHidden}>
                Account Name
              </label>{" "}
              <br />
              <input
