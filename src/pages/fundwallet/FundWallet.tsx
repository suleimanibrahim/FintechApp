...
export default function FundWallet() {
  ...
  let submitRef = useRef<any>(0);

  const handleFunding = function (e: any) {
    e.preventDefault();
    ...
    (async () => {
      ...
      try {
        const response = await axios.post(
          "http://localhost:8085/api/v1/fundLocalWallet",
          { amount: amount },
          config
        );
        await swal("Funding successful!", "success").then(function () {
          window.location.href = "/dashboard";
        });
      } catch (error) {
        console.log(error);
        await swal("Invalid amount", "error");
      }
    })();
  };
        ...
        <div className="fund--wallet-form-body">
          <form className="fund--wallet-form" onSubmit={handleFunding}>
            ...
            <div className="fund--wallet-btn-container">
              <button type="submit" className="fund--wallet-btn">
                Fund Wallet
              </button>
            </div>
          </form>
